...
covid %>% 
  filter(date==max(date)) %>% 
  group_by(county) %>% 
  summarize(countycases = sum(cases, na.rm = TRUE), countydeaths = sum(deaths, na.rm = TRUE)) %>% 
  mutate(Death_ratio = 100* (countydeaths/countycases)) %>% 
  arrange(-Death_ratio) %>% 
  head(5)
```
...
covid %>% 
  filter(date==max(date)) %>% 
  group_by(state) %>% 
  summarize(statecases = sum(cases, na.rm = TRUE), statedeaths = sum(deaths, na.rm = TRUE)) %>% 
  mutate(Death_ratio = 100* (statedeaths/statecases)) %>% 
  arrange(-Death_ratio) %>% 
  head(5)
```
